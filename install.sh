...
#   curl -fsSL https://raw.githubusercontent.com/filterrex-ai/connector-agent/main/install.sh \
#     | bash -s -- --enroll-token 'frc_...' --version v0.1.0-preview.6
#
...
if [ "$HTTP_CODE" = "404" ] || [ "$HTTP_CODE" = "000" ]; then
  ...
  echo "      ghcr.io/filterrex-ai/connector-agent/connector-agent:0.1.0-preview.6"
  echo ""
  echo "  Note: Docker fallback uses the current pinned preview image. For a different"
  echo "  image tag, replace ':0.1.0-preview.6' with the desired version."
  echo ""
